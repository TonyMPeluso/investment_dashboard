...
# --- LP Solver ---
def solve_lp(df, objective_type, weight_cost, discount_rate, budget_cap, ghg_target, reliability_target):
    df = df.copy()
...
app_ui = ui.page_fluid(
    ...
    ui.layout_columns(
        ...
        ui.input_slider("weight_cost", "Cost Weight (= 1 - GHG Weight)", min=0, max=1, value=0.5, step=0.01),
        col_widths=[3, 9]
    ),
...
def server(input, output, session):
    @reactive.Calc
    def df():
        req(input.run_button())  # Only recompute when "Run Scenario" clicked
    ...
    @reactive.effect
    def _():
        if input.objective_type() != "weighted":
            session.send_input_message("weight_cost", {"disabled": True})
        else:
            session.send_input_message("weight_cost", {"disabled": False})
